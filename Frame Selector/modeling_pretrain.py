...
class PretrainVisionTransformerEncoder(nn.Module):
    ...
    def forward_features(self, x, mask):
        _, _, T, _, _ = x.shape  # [batch,channel,time,width,height]
        x = self.patch_embed(x)

        x = x + self.pos_embed.type_as(x).to(x.device).clone().detach()

        B, _, C = x.shape  # 这里处理完后把time width height合并为一个维，然后交换了后两个维度,所以是batch , n , channel, n是twh三个维度撑起来
...
class PretrainVisionTransformer(nn.Module):
    ...
    def forward(self, x, mask, want_middle):
        _, _, T, _, _ = x.shape

        x_vis1 = self.encoder(x, mask)  # [B, N_vis, C_e]
        x_vis = self.encoder_to_decoder(x_vis1)  # [B, N_vis, C_d]  n是没有被mask的帧数（6或者2）*14*14, c 是 embed dim384
        B, N, C = x_vis.shape
        # we don't unshuffle the correct visible token order,
        # but shuffle the pos embedding accorddingly.
        expand_pos_embed = self.pos_embed.expand(B, -1, -1).type_as(x).to(
            x.device).clone().detach()  # 这个是针对所有位置的一个embedding
        pos_emd_vis = expand_pos_embed[~mask].reshape(B, -1, C)
        pos_emd_mask = expand_pos_embed[mask].reshape(B, -1, C)
        x_full = torch.cat([x_vis + pos_emd_vis, self.mask_token + pos_emd_mask],
                           dim=1)  # [B, N, C_d] 这里等于是所有可见的不可见的都加上位置embedding，但要分开加因为
        x = self.decoder(x_full, pos_emd_mask.shape[1])  # [B, N_mask, 3 * 16 * 16]

        if want_middle == 1:
            x_full = x_full.transpose(2, 1)  # 输出应该是1，384，14*14*8

            # x_full=x_full.reshape(x_full.shape[0],x_full.shape[1]*8,14*14)  #1,384*8,14,14
            return x_full
        return x
